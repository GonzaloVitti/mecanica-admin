...
const ServicesPage = () => {
  ...
  const sendQuoteWhatsapp = async (item: WorkOrder) => {
    try {
      const baseUrl = (process.env.NEXT_PUBLIC_API_URL || '').trim()
      const token = localStorage.getItem('accessToken') || localStorage.getItem('token') || ''
      const res = await fetch(`${baseUrl}/api/work-orders/${item.id}/quote_pdf/`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      ...
      // Formatear número argentino para WhatsApp (549XXXXXXXXXX)
      const rawPhone = item.customer_phone?.replace(/\D/g, '') || ''
      let phone = rawPhone
      if (rawPhone && !rawPhone.startsWith('549') && !rawPhone.startsWith('54')) {
        // Quitar 0 inicial si existe, agregar código de país Argentina
        phone = '549' + rawPhone.replace(/^0/, '')
      }

      const vehicle = [item.vehicle_brand, item.vehicle_model, item.vehicle_license_plate].filter(Boolean).join(' ')
      const message = `Hola${item.customer_name ? ` ${item.customer_name}` : ''}! Te enviamos el presupuesto *${item.work_order_number}*${vehicle ? ` por tu vehículo ${vehicle}` : ''}. Quedamos a disposición ante cualquier consulta.`
      ...
      showAlert('info', 'WhatsApp', 'PDF descargado. Adjuntalo en la conversación de WhatsApp que se abrió.')
    } catch {
      showAlert('error', 'Error', 'Error al preparar el envío por WhatsApp')
    }
  }
                    ...
                    <TableRow key={item.id}>
                      ...
                      <TableCell className="px-4 py-3 text-theme-sm">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <button
                            onClick={() => sendQuoteWhatsapp(item)}
                            title="Enviar presupuesto por WhatsApp"
                            className="inline-flex items-center justify-center w-7 h-7 rounded-md bg-[#25D366] hover:bg-[#1ebe5d] text-white transition-colors"
                          >
